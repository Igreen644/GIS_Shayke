...
public class KmlWriter {
			...
			for (int i = 0; i < list.size(); i++) {
				if (!list.get(i).wifiList.isEmpty()) {
					for (int j = 0; j < list.get(i).wifiList.size(); j++) {
						...
						desc.appendChild(doc.createTextNode("SSID: " + list.get(i).wifiList.get(j).getSsid() + "\n"
								+ "MAC: " + list.get(i).wifiList.get(j).getMac() + "\n" + "Signal: "
								+ list.get(i).wifiList.get(j).getSignal() + "\n" + "Time: "
								+ list.get(i).date.toString() + "\n" + "Latitude: "
								+ list.get(i).getCoordinates().getLatitude() + "\n" + "Longitude: "
								+ list.get(i).getCoordinates().getLongitude() + "\n" + "Altitude: "
								+ list.get(i).getCoordinates().getAltitude()));
		...
		public static String addFilteringArea(double[] rectTop, double[] rectBot, String kml) {// adds the rectangle of
																								// the filtering area
